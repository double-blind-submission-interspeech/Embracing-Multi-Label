...
class ModelWrapper():
    ...
    def init_model(self):
        ...
        assert self.model_type in [
            ...
            "wav2vec2-base", "wav2vec2-base-960h","wav2vec2-large", "wav2vec2-large-robust",
        ...
        self.ser_model = ser.ClassificationHead(
            idim,
            self.hidden_dim, 
            self.num_layers, 
            self.output_num,
    ...
    def feed_forward(self, x, eval=False, **kwargs):
        ...
        def __inference__(self, x, **kwargs):
            ...
            if self.use_chunk:
                h = utils.DynamicChunkForAll(w2v, self.chunk_window, self.chunk_num, 1)
                h = self.chunk_model(h)
            else:
                h = utils.AverageAll(w2v)
