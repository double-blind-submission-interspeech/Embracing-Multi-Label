...
def mr_all_rule_loss(input,target,weights,rule_weights,reduction='mean'):
    """
    :param input: (batch, *)
    :param target: (batch, *) same shape as input, each item must be a valid distribution: target[i, :].sum() == 1.
    :weights from the training set: [0.2139, 0.3445, 0.4665, 3.3741, 0.3337, 2.2446, 0.2398, 1.3928, 0.3900]
    ...
    for idx, each in enumerate(target):
        cur_train = target[idx,:] # [0.01     0.532222 0.01     0.323333 0.114444 0.01    ]
        max_index = torch.where(torch.max(cur_train)==cur_train)[0]
        # print("Loss: ",cur_train,max_index,cur_train.shape)
        if len(max_index) == 1 and torch.max(cur_train)>0.5:
                one_hot_targt = torch.zeros(target[idx,:].shape).cuda()
                one_hot_targt[max_index]=1
                eps = 0.05
                one_hot_targt = one_hot_targt * (1 - eps) + (1-one_hot_targt) * eps / (len(one_hot_targt) - 1)                
                mr_t.append(one_hot_targt)
                # print(one_hot_targt,target[idx,:])
                # mr_t.append(target[idx,:])
                mr_p.append(input[idx,:])
        elif len(max_index) == 1:
                one_hot_targt = torch.zeros(target[idx,:].shape).cuda()
                one_hot_targt[max_index]=1
                eps = 0.05
                one_hot_targt = one_hot_targt * (1 - eps) + (1-one_hot_targt) * eps / (len(one_hot_targt) - 1)  
                pr_t.append(one_hot_targt)
                # print(one_hot_targt,target[idx,:])
                # pr_t.append(target[idx,:])
                pr_p.append(input[idx,:])
        else:
                ar_t.append(target[idx,:])
                ar_p.append(input[idx,:])

    # MR loss
    if len(mr_t)!=0:
        mr_t = torch.stack(mr_t, dim=0)
        mr_p = torch.stack(mr_p, dim=0)        
        mr_loss = class_balanced_softmax_cross_entropy_with_softtarget(mr_p,mr_t,weights,reduction='mean')
    else:
        mr_loss = torch.tensor([0]).cuda()

    # PR loss
    if len(pr_t)!=0:
        pr_t = torch.stack(pr_t, dim=0)     
        pr_p = torch.stack(pr_p, dim=0)
        pr_loss = class_balanced_softmax_cross_entropy_with_softtarget(pr_p,pr_t,weights,reduction='mean')
    else:
        pr_loss = torch.tensor([0]).cuda()

    # AR loss
    if len(ar_t)!=0:
        ar_t = torch.stack(ar_t, dim=0)
        ar_p = torch.stack(ar_p, dim=0)     
        ar_loss = class_balanced_softmax_cross_entropy_with_softtarget(ar_p,ar_t,weights,reduction='mean')
    else:
        ar_loss = torch.tensor([0]).cuda()

    loss = mr_loss*rule_weights[0] + pr_loss*rule_weights[1] + ar_loss*rule_weights[2]

    return loss

def all_rule_loss(input,target,weights,rule_weights,reduction='mean'):
    """
    :param input: (batch, *)
    :param target: (batch, *) same shape as input, each item must be a valid distribution: target[i, :].sum() == 1.
    :weights from the training set: [0.2139, 0.3445, 0.4665, 3.3741, 0.3337, 2.2446, 0.2398, 1.3928, 0.3900]
    ...
    loss = mr_loss*rule_weights[2] + pr_loss*rule_weights[1] + ar_loss*rule_weights[0]
